...
public class IngredientDAO {
    ...
    public void update(int addgrCoffee, int addfreshMilk, int addcondensedMilk, int addsugar, int addmlTea, int addmlSyrup )  {
		...
		try {
				...
				// update database
				String add_sql = "UPDATE `ingredient` SET `COFFEE(GR)`='" +grCoffee+"', `FRESH MILK(ML)`='"+freshMilk+"', `CONDENSED MILK(ML)`='"+condensedMilk+"', `SUGAR(ML)`='"+sugar+"', `AMOUNT OF TEA (ML)`='"+mlTea+"', `AMOUNT OF SYRUP (ML)`='"+mlSyrup+"' WHERE  `id`=0;";
